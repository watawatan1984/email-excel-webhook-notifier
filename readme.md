# 有害鳥獣捕獲依頼メール自動処理システム 保守・運用マニュアル

このドキュメントは、本システムの概要、導入方法、日常の運用、およびトラブルシューティングについて解説する管理者向けマニュアルです。
Microsoft Graph API移行に対応しています。

## 1. システム概要
本システムは、自治体等からの有害鳥獣捕獲依頼（メール）を自動処理し、データベースおよび通知システムへ連携するPythonスクリプトです。

### 処理フロー
1.  **メール受信**: サーバー上で定期実行され、指定された差出人（市役所・役場等）からのメールをMicrosoft Graph API経由で取得します。
2.  **Excel抽出**: メールに添付されたExcelファイル（パスワード付き）をダウンロードし、復号・解析します。
3.  **データ連携**:
    *   **WordPress**: 抽出した依頼情報をREST API経由で登録します。
    *   **Microsoft Teams**: 処理結果の概要をTeamsチャネルに通知します。
    *   **OneSignal**: スマートフォンアプリ向けにプッシュ通知を送信します。
